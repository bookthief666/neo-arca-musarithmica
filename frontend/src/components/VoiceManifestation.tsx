...
/**
 * The revelation is a folio the Arca prints and pushes out of the carriage:
 * a sheet of vellum ruled into four ink lanes running left to right in event
 * order. Relative duration is the lane cell's own width weight plus an explicit
 * minim count. No vertical position carries meaning — nothing here implies a
 * register. The sheet keeps full table semantics for assistive technology.
 */
export function VoiceManifestation({ execution, onReturn }: VoiceManifestationProps) {
  ...
  return (
    <section className="voice-manifestation" aria-labelledby="voices-title" aria-live="polite">
      <span className="voice-manifestation__slot" aria-hidden="true" />

      <div className="voice-folio">
        <span className="voice-folio__deckle" aria-hidden="true" />
