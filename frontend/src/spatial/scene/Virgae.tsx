...
import { CARRIER_READING_POSE } from './stations'

/** One persistent carrier mesh animated through cell → hand → workspace. */
export function Virgae({manifest,state,materials,nextAffordance,travelRef,onRetrieveCarrier,onPlaceCarrier}: {
  ...
  useFrame((_,delta) => {
    ...
    else if(location==='hand') goal.current.set(0.055,DECK_TOP+0.072,0.055)
    else goal.current.set(
      0,
      CARRIAGE.y+CARRIAGE.wall+VIRGA.thickness/2+0.0008,
      CARRIER_READING_POSE.z+travelRef.current,
    )
    ...
    if(destination.current) destination.current.position.z=travelRef.current+CARRIER_READING_POSE.z
  })

  return <group>
    <group ref={group} position={[0,storedY,0]}>
      <Virga
        source={manifest.critical_edition_carrier}
        materials={materials}
        held={location==='hand'}
        cued={nextAffordance==='retrieve_carrier'}
        onPointerDown={event => {
    ...
    {location==='hand' && <mesh
      ref={destination}
      position={[0,CARRIAGE.y+CARRIAGE.wall+0.001,travelRef.current+CARRIER_READING_POSE.z]}
      onPointerDown={event => {event.stopPropagation();onPlaceCarrier()}}
    >
      <boxGeometry args={[CARRIAGE.activeChannelWidth,0.004,VIRGA.length+0.008]} />
      <meshStandardMaterial color="#bba874" transparent opacity={0.42} />
    </mesh>}
  </group>
}
